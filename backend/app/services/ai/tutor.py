...
from .providers import MockLLMProvider, get_provider
from .base import BaseLLMProvider

class AITutorService:
    def __init__(self, provider: Optional[BaseLLMProvider] = None):
        # Allow pluggable model provider (defaults to active provider based on keys)
    ...
    def start_chat(self, db: Session, user_id: int, context_type: str, context_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        welcome = provider.generate_text(
    ...
    def continue_chat_stream(
        ...
        try:
            for chunk in provider.generate_stream(
                ...
                reply_accumulated.append(chunk)
                yield chunk
        finally:
            ...
            ai_log = AIChat(
                ...
                context_type=context_type,
                context_id=context_id,
                message_text=full_reply if full_reply else "Error yielding stream.",
                sender="ai"
            )
            db.add(ai_log)
            db.commit()
    ...
    def generate_explanation(self, db: Session, user_id: int, concept: str) -> str:
        system = self._assemble_system_prompt(db, user_id)
        return self.get_active_provider().generate_text(
            system_prompt=system,
